...
`transcript.md` is a metadata stub in this milestone, because there is no interview content to render yet and no renderer to render it. The walking skeleton replaces its body.
...
* *THEN* the file `<root>/<id>/transcript.md` MUST exist after the first save
* *AND* it MUST name the session's identifier and both timestamps
* *AND* it MUST contain nothing beyond that header, because this milestone stores no interview content
* *AND* the second save MUST rewrite it to name the second revision's `updatedAt`, so a completed save leaves behind no transcript describing an earlier revision
